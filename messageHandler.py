    ...
    async def getPDF(self, message: types.Message, state: FSMContext):
        ...
        for paragraph in paragraphs:
            ...
            if 'delo_data2' in paragraph.text:
                paragraph.text = paragraph.text.replace('delo_data2', data['DATE'])
        ...
        if os.path.exists(f'./docks/{a}.pdf'):
            os.remove(f'./docks/{a}.pdf')
        if os.path.exists(f'./docks/{a}.docx'):
            os.remove(f'./docks/{a}.docx')
